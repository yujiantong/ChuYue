...
(function ($) {
    $.extend({
    	...
    	table: {
			...
			colresizable: function(){
				$("#bootstrap-table").colResizable({
					...
					//onResize: onSampleResized          // 在行下面展示其他数据列表
				});
        ...
        treeTable: {
            ...
            //表格列拖动方法
            colresizable: function(){
                $("#bootstrap-tree-table").colResizable({
                    liveDrag: true,//实时显示滑动位置
                    gripInnerHtml: "<div class='grip'></div>",
                    draggingClass: "dragging",
                    postbackSafe: false,//刷新后保留之前的拖拽宽度
        ...
        operate: {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.chuyue_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
		...
		reporttable: {
			...
			init: function(options) {
				var defaults = {
					...
					rightFixedColumns: false,
					rightFixedNumber: 0,
					queryParams: $.reporttable.queryParams,
					rowStyle: {},
				};
				var options = $.extend(defaults, options);
				$.reporttable._option = options;
				$.btTable = $('#' + options.id);
				$.reporttable.initEvent();

				/*var searchdata ={};//将所有参数统一封装
				var CONDITION = {};
				CONDITION = $.reporttable.queryParams;
				var beforecondition = $.reporttable.condition;//拼装参数
				$.each(beforecondition, function(key) {
					CONDITION[key] = beforecondition[key];
				});*/
				/*var currentId = $.common.isEmpty($.reporttable._option.formId) ? $('form').attr('id') : $.reporttable._option.formId;
				var params = $.common.isEmpty($.reporttable._option.tableId) ? $.btTable.bootstrapTable('getOptions') : $("#" + $.reporttable._option.tableId).bootstrapTable('getOptions');
				var search = $.common.formToJSON(currentId);
				if($.common.isNotEmpty($.reporttable._option.data)){
					$.each($.reporttable._option.data, function(key) {
						search[key] = $.reporttable._option.data[key];
					});
				}
				search.pageSize = params.limit;
				search.pageNum =  params.offset / params.limit + 1;
				...
				$.each(condition, function(key) {
					search[key] = condition[key];
				});*/
				//searchdata.CONDITION = JSON.stringify(CONDITION);//将所有查询条件统一分装
				...
				$('#' + options.id).bootstrapTable({
					...
					rowStyle: options.rowStyle,                         // 通过自定义函数设置行样式
					columns: options.columns,                           // 显示列信息（*）
					responseHandler: $.reporttable.responseHandler,           // 在加载服务器发送来的数据之前处理函数
					onLoadSuccess: $.reporttable.onLoadSuccess,               // 当所有数据被加载时触发处理函数
					exportOptions: options.exportOptions,               // 前端导出忽略列索引
					detailFormatter: options.detailFormatter,           // 在行下面展示其他数据列表
				});
			},
			...
			// 请求获取数据后处理回调函数
			responseHandler: function(res) {
				if (typeof $.reporttable._option.responseHandler == "function") {
					$.reporttable._option.responseHandler(res);
				}
				if (res.code == 0) {
					if ($.common.isNotEmpty($.reporttable._option.sidePagination) && $.reporttable._option.sidePagination == 'client') {
						return res.rows;
					} else {
						if ($.common.isNotEmpty($.reporttable._option.rememberSelected) && $.reporttable._option.rememberSelected) {
							var column = $.common.isEmpty($.reporttable._option.uniqueId) ? $.reporttable._option.columns[1].field : $.reporttable._option.uniqueId;
							$.each(res.rows, function(i, row) {
								row.state = $.inArray(row[column], selectionIds) !== -1;
							})
						}
			...
			initEvent: function(data) {
				...
				$.btTable.on("check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table load-success.bs.table", function () {
					// 工具栏按钮控制
					var rows = $.common.isEmpty($.reporttable._option.uniqueId) ? $.reporttable.selectFirstColumns() : $.reporttable.selectColumns($.reporttable._option.uniqueId);
					// 非多个禁用
					$('#' + $.reporttable._option.toolbar + ' .multiple').toggleClass('disabled', !rows.length);
					// 非单个禁用
					$('#' + $.reporttable._option.toolbar + ' .single').toggleClass('disabled', rows.length!=1);
				});
				// 绑定选中事件、取消事件、全部选中、全部取消
				$.btTable.on("check.bs.table check-all.bs.table uncheck.bs.table uncheck-all.bs.table", function (e, rows) {
					// 复选框分页保留保存选中数组
					var rowIds = $.reporttable.affectedRowIds(rows);
					if ($.common.isNotEmpty($.reporttable._option.rememberSelected) && $.reporttable._option.rememberSelected) {
						func = $.inArray(e.type, ['check', 'check-all']) > -1 ? 'union' : 'difference';
						selectionIds = _[func](selectionIds, rowIds);
					}
			...
			// 当所有数据被加载时触发
			onLoadSuccess: function(data) {
				if (typeof $.reporttable._option.onLoadSuccess == "function") {
					$.reporttable._option.onLoadSuccess(data);
				}
				// 浮动提示框特效
				$("[data-toggle='tooltip']").tooltip();
			},
			// 表格销毁
			destroy: function (tableId) {
				var currentId = $.common.isEmpty(tableId) ? $.reporttable._option.id : tableId;
				$("#" + currentId).bootstrapTable('destroy');
			},
			// 序列号生成
			serialNumber: function (index) {
			...
			/**
			 * 点击查询按钮前 拼装参数  yujiantong
			 * @param condition
			 */
			beforeSearch : function(condition){
				$.reporttable.condition = condition;
			},
			/**
			 * 搜索-默认第一个form
			 * 2019-10-16  yujiantong
			 * @param formId
			 * @param tableId
			 * @param data
			 */
			search: function(formId, tableId, data) {
				...
				params.queryParams = function(params) {
					...
					search.CONDITION = JSON.stringify(search);//将所有查询条件统一分装
					return search;
				}
				if($.common.isNotEmpty(tableId)){
					$("#" + tableId).bootstrapTable('refresh', params);
			...
			// 下载模板
			importTemplate: function() {
				$.get($.reporttable._option.importTemplateUrl, function(result) {
					if (result.code == web_status.SUCCESS) {
						window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
					} else if (result.code == web_status.WARNING) {
						$.modal.alertWarning(result.msg)
			...
			importExcel: function(formId) {
				...
				layer.open({
					...
					maxmin: true,
					shade: 0.3,
					title: '导入' + $.reporttable._option.modalName + '数据',
					content: $('#' + currentId).html(),
					btn: ['<i class="fa fa-check"></i> 导入', '<i class="fa fa-remove"></i> 取消'],
					// 弹层外区域关闭
					shadeClose: true,
					btn1: function(index, layero){
						...
						formData.append("updateSupport", $("input[name='updateSupport']").is(':checked'));
						$.ajax({
							url: $.reporttable._option.importUrl,
							data: formData,
							cache: false,
							contentType: false,
							processData: false,
			...
			selectFirstColumns: function() {
				var rows = $.map($.btTable.bootstrapTable('getSelections'), function (row) {
					return row[$.reporttable._option.columns[1].field];
				});
				if ($.common.isNotEmpty($.reporttable._option.rememberSelected) && $.reporttable._option.rememberSelected) {
					rows = rows.concat(selectionIds);
				}
